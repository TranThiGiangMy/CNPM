...
class Province(BaseModel):
    ...
    wards = relationship('Ward', backref='province', lazy=True)
...
class Ward(BaseModel):
    ...
    airports = relationship('Airport', backref='ward', lazy=True)
...
class Airport(BaseModel):
    ...
    active = Column(Boolean, default=True)
    airplanes = relationship('Airplane', backref='airport', lazy=True)

    start_locations = relationship('Route', foreign_keys='Route.start_location', backref='airport1', lazy=True)
    end_locations = relationship('Route', foreign_keys='Route.end_location', backref='airport2', lazy=True)
...
class TypeOfPosition(BaseModel):
    ...
    active = Column(Boolean, default=True)
    # bookings = relationship('Booking', backref='type_of_position', lazy=True)
...
class Flight(BaseModel):
    ...
    airplane = Column(Integer, ForeignKey('airplane.id'), nullable=False)
    pilot = Column(Integer, ForeignKey('user.id'), nullable=False)
    start_time = Column(DateTime)
    end_time = Column(DateTime)
    gate = Column(String(10))
    delay = Column(String(10), default=0)
...
class Pricing(BaseModel):
    ...
    flights = relationship('Flight', backref='pricing', lazy=True)
...
class Booking(BaseModel):
    ...
    type_of_position = Column(Integer, ForeignKey('type_of_position.id'), nullable=False)
...
if __name__ == "__main__":
    ...
    #          avatar='https://res.cloudinary.com/dv2y8ynkx/image/upload/v1671088051/ow7cu0ziw21e20o1fpk1.jpg')
    # db.session.add(u)
    # db.session.commit()
